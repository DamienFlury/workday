...
import {
  QuoteState, FETCH_QUOTE_PENDING, FETCH_QUOTE_REJECTED, FETCH_QUOTE_FULFILLED,
} from '../store/actions/quote-actions';
...
const QuoteProvider: React.FC = ({ children }) => {
  ...
  const fetchQuote = async () => {
    ...
    try {
      ...
      const newQuote = data.contents ? data.contents.quotes[0] : { quote: 'Don\'t cry because it\'s over, smile because it happened', author: 'Dr. Seuss' };
      dispatch({ type: FETCH_QUOTE_FULFILLED, quote: newQuote });
    } catch (error) {
